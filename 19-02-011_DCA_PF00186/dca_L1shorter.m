%function dca(inputfile, outputfile, Woutputfile, MUoutputfile)
function dca(inputfile,l1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [M,N,q,align] = load_alignments(inputfile,l1);
%   [N,M,q,align] = return_alignment(inputfile);
    ...
    fp = fopen('result_DI.dat','w');
    [W,MU1,MU2]=Compute_Results(Pij, Pi,Pij_true, Pi_true, invC, N, q,fp);
    fclose(fp); 
    
    %fp = fopen(Woutputfile,'w');
    fp = fopen('result_W.dat','w');
    for i=1:(N-1)
        for j=(i+1):N
            for k=1:q
                for l=1:q
    ...
    %fp = fopen(MUoutputfile,'w');
    fp = fopen('result_MU.dat','w');
    for i=1:(N-1)
        for j=(i+1):N
            for k=1:q
                fprintf(fp,'%d %d %d %g %g\n', i, j, k, MU1(i,j,k), MU2(i,j,k));
            end
        end
    end
    fclose(fp); 
...
function [M,N,q,Z]=load_alignments(inputfile,l1)
    Z=importdata(inputfile);
    Z=Z(1:l1-1,:);
    %Z(l1:end,:)=[];
    Z=Z+1;
    [M,N]=size(Z);
...
%    align_full = myfastaread(inputfile);
%    M = length(align_full);
...
function [W,MU1,MU2]=Compute_Results(Pij,Pi,Pij_true,Pi_true,invC, N,q, fp)
% computes and prints the mutual and direct informations
    ...
    for i=1:(N-1)
        for j=(i+1):N
            ...
            [DI_mf_pc, mu1, mu2] = bp_link(i,j,W_mf,Pi,q);
...
function [DI, mu1, mu2] = bp_link(i,j,W,P1,q)
