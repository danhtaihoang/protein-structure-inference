...
#------------------------------
def remove_gap_cols(s,fg=0.2):
# remove positions having a fraction fg of gaps
    l,n = s.shape
    # gap positions:
    frequency = [(s[:,i] == '-').sum()/float(l) for i in range(n)]
    gap_cols = [i for i in range(n) if frequency[i] > fg]

    return np.delete(s,gap_cols,axis=1),np.array(gap_cols)

#------------------------------
def remove_conserved_cols(s,fc=0.8):
...
s = remove_bad_seqs(s,0.2)
print(s.shape)

print('remove gap cols')
s,gap_cols = remove_gap_cols(s,fg=0.2)
print('number of gap columns removed:',gap_cols.shape[0])
print(s.shape)
...
for i in range(s.shape[1]):
    s[:,i] = replace_lower_by_higher_prob(s[:,i],p0=0.004)

# remove conserved cols
print('remove conserved columns')
s,conserved_cols = remove_conserved_cols(s,fc=0.8)
print(s.shape)
